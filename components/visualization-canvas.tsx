...
function resolveFrequencyProfileGain(frequencyHz: number) {
	const bass = Math.exp(-Math.pow((frequencyHz - 110) / 210, 2));
	const mid = Math.exp(-Math.pow((frequencyHz - 1400) / 1500, 2));
	const treble = Math.exp(-Math.pow((frequencyHz - 7600) / 4200, 2));

	return 1 + bass * 0.42 + mid * 0.24 + treble * 0.28;
}

function analyzeFrequencyBands(data: Uint8Array, sampleRate: number, settings: SpectrumSettings, bandCount: number) {
	const nyquist = sampleRate / 2;
	const safeCount = Math.max(1, bandCount);
	const bands = new Float32Array(safeCount);
	if (!data.length) return bands;

	const safeStart = Math.max(20, Math.min(settings.startFrequency, nyquist - 1));
	const safeEnd = Math.max(safeStart + 1, Math.min(settings.endFrequency, nyquist));

	const startLog = Math.log10(safeStart);
	const endLog = Math.log10(safeEnd);

	for (let i = 0; i < safeCount; i += 1) {
		const t0 = i / safeCount;
		const t1 = (i + 1) / safeCount;
		const hz0 = Math.pow(10, startLog + (endLog - startLog) * t0);
		const hz1 = Math.pow(10, startLog + (endLog - startLog) * t1);

		const i0 = Math.max(0, Math.min(data.length - 1, Math.floor((hz0 / nyquist) * data.length)));
		const i1 = Math.max(i0 + 1, Math.min(data.length, Math.ceil((hz1 / nyquist) * data.length)));

		let sum = 0;
		for (let index = i0; index < i1; index += 1) {
			sum += (data[index] ?? 0) / 255;
		}
		const avg = sum / Math.max(1, i1 - i0);
		const centerHz = (hz0 + hz1) / 2;
		const gain = resolveFrequencyProfileGain(centerHz);
		bands[i] = Math.min(1.25, Math.pow(avg * settings.sensitivity * gain, 0.78));
	}
...
function drawBars(ctx: CanvasRenderingContext2D, data: Uint8Array, width: number, height: number, sampleRate: number, settings: SpectrumSettings, displayState: Float32Array, deltaMs: number) {
	const count = Math.max(16, settings.frequencyBands);
	const analyzed = analyzeFrequencyBands(data, sampleRate, settings, count);
	...
	for (let i = 0; i < count; i += 1) {
		const value = Math.max(0, Math.min(1, analyzed[i] ?? 0));
...
function drawCircular(ctx: CanvasRenderingContext2D, data: Uint8Array, width: number, height: number, sampleRate: number, settings: SpectrumSettings) {
	...
	const analyzed = analyzeFrequencyBands(data, sampleRate, settings, bars);
	...
	for (let i = 0; i < bars; i += 1) {
		const energy = Math.max(0, Math.min(1, analyzed[i] ?? 0));
		const angle = (i / bars) * sweep - Math.PI / 2;
		...
		let hue;
		if (settings.colorScheme === "neon") hue = (i / bars) * 360 + 180;
...
function buildSpectrumEnvelope(data: Uint8Array, sampleRate: number, settings: SpectrumSettings, points: number) {
	const sampledBands = Math.max(32, settings.frequencyBands);
	const bands = analyzeFrequencyBands(data, sampleRate, settings, sampledBands);
	const values = new Float32Array(points);

	for (let i = 0; i < points; i += 1) {
		const ratio = i / Math.max(1, points - 1);
		const source = ratio * (sampledBands - 1);
		const left = Math.floor(source);
		const right = Math.min(sampledBands - 1, left + 1);
		const t = source - left;
		values[i] = (bands[left] ?? 0) * (1 - t) + (bands[right] ?? 0) * t;
	}
