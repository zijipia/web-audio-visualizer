...
function drawOverlayText(
	...
	const pulse = 1 + bassIntensity * 0.08;
	ctx.save();
	ctx.translate(width / 2, height * 0.82);
	ctx.scale(pulse, pulse);
	ctx.textAlign = "center";
	ctx.textBaseline = "middle";
	ctx.font = `700 ${Math.max(14, Math.floor(textSize))}px Inter, system-ui, sans-serif`;
	ctx.shadowColor = `rgba(255, 255, 255, ${0.28 + bassIntensity * 0.35})`;
	ctx.shadowBlur = 10 + bassIntensity * 24;
...
export function VisualizationCanvas({ frequencyData, timeData, sampleRate, mode, duration, currentTime, onSeek, onExportCanvasReady, settings, bassIntensity, backgroundImage, overlayText, overlayTextColor, overlayTextSize }: VisualizationCanvasProps) {
	...
	useEffect(() => {
		...
		const renderFrame = () => {
			...
			const draw = (ctx: CanvasRenderingContext2D, width: number, height: number, particles: Particle[]) => {
				...
				drawOverlayText(ctx, width, height, overlayText, overlayTextColor, overlayTextSize, bassIntensity);
	...
	}, [mode, frequencyData, timeData, duration, currentTime, sampleRate, settings, bassIntensity, overlayText, overlayTextColor, overlayTextSize]);
